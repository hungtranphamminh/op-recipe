...
export default function SecondRecipeColumn({ recipes }: { recipes: Recipe[] }) {
  ...
  return (
    <div className="w-[400px] flex flex-col items-center justify-start gap-[30px]">
      {recipes.map((recipe: Recipe, index: number) => (
        <article
          key={index}
          className="w-[400px] flex flex-col transition-all duration-300 group items-center justify-center relative mt-5 "
        >
          {/* main image */}
          <div
            className={`w-[400px] relative z-10 aspect-w-7 ${
              index === 1 ? "aspect-h-5" : "aspect-h-6"
            }`}
          >
            <Image
              src={recipe.imgUrl}
